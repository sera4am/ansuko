...
const ansukoGeoPlugin = (ansuko) => {
    ...
    const unionPolygon = (geo, digit) => {
        ...
        const g = geo;
        if (Array.isArray(geo)) {
            list = geo.map(g => toPolygonGeoJson(g, digit));
        }
        else if (_.has(geo, "type")) {
            switch (_.get(g, "type")?.toLowerCase()) {
                case "featurecollection":
                    list = g.features?.map((f) => toPolygonGeoJson(f, digit));
                    break;
                case "feature":
                    if (g.geometry?.type !== "polygon") {
                        ...
                        list = g.geometry?.coordinates.map((c) => toPolygonGeoJson(c, digit));
                    }
                    break;
                case "polygon":
                    return g;
                case "multipolygon":
                    list = g.coordinates.map((c) => toPolygonGeoJson(c, digit));
                    break;
                default:
                    return null;
            }
